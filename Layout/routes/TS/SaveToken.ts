...
router.post('/Add', (req : any , res  : any ) => {
  ...
  if (!Name || !Email)  return res.status(400).json({ error: 'Name and Age are required' });
  const token = jwt.sign({ ID : ID , Name : Name, Email: Email }, SECRET_KEY, { expiresIn: '1h' });
  res.json({ token });
});


// Get Token
router.get('/GETTOKEN', (req : any , res : any) => {
